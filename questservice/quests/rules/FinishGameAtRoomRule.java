...
public class FinishGameAtRoomRule implements QuestRule{
    Logger logger = LoggerFactory.getLogger(FinishGameAtRoomRule.class);
    ...
    public void check(QuestModel model, String info, int day) {
        try {
            ...
            if (categoryInfo.getBet() != model.getPremise()) {
                return;
            }
           ...
           if (questController.invalidQuestDay(model)) return;
            ...
            logger.error("Error at FinishGameAtRoomRule>> "+e.getMessage());
        }
    }
}

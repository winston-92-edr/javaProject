...
public class LostSingleGameGainXpRule implements QuestRule{
    Logger logger = LoggerFactory.getLogger(LostSingleGameGainXpRule.class);
    ...
    public void check(QuestModel model, String info, int day) {
        try {
            ...
            questController.updateUserXp(model, user, quest, ExpSourceType.SINGLE);

        } catch (Exception e) {
            logger.error("Error at LostSingleGameGainXpRule>> " + e.getMessage());
        }
    }
}

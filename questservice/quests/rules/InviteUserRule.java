...
public class InviteUserRule implements QuestRule {
    Logger logger = LoggerFactory.getLogger(InviteUserRule.class);
    ...
    public void check(QuestModel model, String info, int day) {
        try {
            ...
            if (socialCategoryInfo.getSocialActionType() != SocialActionType.INVITE_FRIEND_TABLE.getValue()) return;
           ...
           if (questController.invalidQuestDay(model)) return;
            ...
            logger.error("Error at InviteUserRule>> " + e.getMessage());
        }
    }
}

...
public class WonGameLongestTileRule implements QuestRule {
    Logger logger = LoggerFactory.getLogger(WonGameLongestTileRule.class);
    ...
    public void check(QuestModel model, String info, int day) {
        try {
            ...
            if (categoryInfo.getLongestTile() < model.getPremise()) return;
           ...
           if (questController.invalidQuestDay(model)) return;
            ...
            logger.error("Error at WonGameLongestTileRule>> " + e.getMessage());
        }
    }

...
public class DifferentDaysLoginRule implements QuestRule {
    private Logger logger = LoggerFactory.getLogger(DifferentDaysLoginRule.class);

    @Override
    public void check(QuestModel model, String info, int day) {
        try {
            DifferentDaysCategoryInfo categoryInfo = QuestController.getGson().fromJson(info, DifferentDaysCategoryInfo.class);

            if (categoryInfo.getType() != DifferentDaysType.LOGIN.getValue()) {
                return;
            }
            ...
            int lastDay = user.getLastLoginDay();
            int seasonDay = questController.getSeason().getDay();

            if (lastDay == seasonDay) return;
            else {
                questController.updateUserDifferentDaysLogin(user);
            }
            ...
            logger.error("Error at DifferentDaysLoginRule>> " + e.getMessage());
        }
    }
}

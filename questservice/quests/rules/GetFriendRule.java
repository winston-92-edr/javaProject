...
public class GetFriendRule implements QuestRule {
    Logger logger = LoggerFactory.getLogger(GetFriendRule.class);
    ...
    public void check(QuestModel model, String info, int day) {
        try {
            ...
            if (socialCategoryInfo.getSocialActionType() != SocialActionType.ADD_FRIEND.getValue()) return;
           ...
           if (questController.invalidQuestDay(model)) return;
            ...
            logger.error("Error at GetFriendRule>> " + e.getMessage());
        }
    }
}

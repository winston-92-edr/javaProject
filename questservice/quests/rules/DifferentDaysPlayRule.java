...
public class DifferentDaysPlayRule implements QuestRule {
    Logger logger = LoggerFactory.getLogger(DifferentDaysPlayRule.class);
    ...
    public void check(QuestModel model, String info, int day) {
        try {
            ...
            if (categoryInfo.getType() != DifferentDaysType.PLAY_GAME.getValue()) {
                return;
            }
            ...
            int lastDay = user.getLastPlayDay();
            int seasonDay = questController.getSeason().getDay();

            if (lastDay == seasonDay) return;
            else {
                questController.updateUserDifferentDaysPlay(user);
            }
            ...
            logger.error("Error at DifferentDaysPlayRule>> " + e.getMessage());
        }
    }
}

...
public class PairedEarnMoneyRule implements QuestRule {
    Logger logger = LoggerFactory.getLogger(PairedEarnMoneyRule.class);
    ...
    public void check(QuestModel model, String info, int day) {
        try {
           ...
           if (questController.invalidQuestDay(model)) return;
            ...
            logger.error("Error at PairedEarnMoneyRule>> " + e.getMessage());
        }
    }
}

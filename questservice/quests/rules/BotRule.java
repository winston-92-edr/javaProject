...
import com.mynet.questservice.quests.category.BotCategoryInfo;
...
public class BotRule implements QuestRule {
    Logger logger = LoggerFactory.getLogger(BotRule.class);
    ...
    public void check(QuestModel model, String info, int day) {
        try {
            ...
            BotCategoryInfo categoryInfo = QuestController.getGson().fromJson(info, BotCategoryInfo.class);
           ...
           if (questController.invalidQuestDay(model)) return;
            ...
            logger.error("Error at BotRule>> " + e.getMessage());
        }
    }
}

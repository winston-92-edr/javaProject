...
public class ChatRequest extends ObserverRunnable {
    ProxyUser user;
    NetworkMessage request;
    String id;

    public ChatRequest() {
        this.id = "ChatRequest_" + System.currentTimeMillis();
    }
    ...
    public void run() {
        try {
            Node node = UserController.getInstance().getNodeController().getNode(NodesType.chat);
            if (node != null) {

...
public class BonusRequest extends ObserverRunnable {
    ProxyUser user;
    NetworkMessage request;
    String id;

    public BonusRequest() {
        this.id = "BonusRequest_" + System.currentTimeMillis();
    }
    ...
    public void run() {
        try {
            Node node = UserController.getInstance().getNodeController().getNode(NodesType.bonus);
            if (node != null) {
